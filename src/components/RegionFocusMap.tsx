...
export function RegionFocusMap({ slug, regionName, destinations }: Props) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setSize({ w, h: Math.max(320, w * 0.62) })
    }
    measure()
  ...
  const { pathD, markers } = useMemo(() => {
    if (!regionFeature) {
      return { pathD: '', markers: [] as Marker[] }
    }
  ...
  }, [regionFeature, destinations, size.w, size.h])
  ...
  return (
    <section className="rounded-3xl border border-[var(--border)] bg-gradient-to-b from-[var(--surface-2)] to-white p-4 shadow-[var(--shadow-soft)] md:p-8">
      <div className="mb-5 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <h2 className="font-display text-2xl font-semibold text-[var(--forest-deep)] md:text-3xl">
            Regional map and places
          </h2>
          <p className="mt-2 max-w-2xl text-sm text-[var(--muted)] md:text-base">
            {regionName} is shown as a full map view. Markers are built from your
            destination Google Maps data.
          </p>
        </div>
        {hoveredMarker && (
      ...
      <div ref={wrapRef} className="relative w-full">
        <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--mist)]/40">
          {loadError && (
            ...
            >
              <defs>
                <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#3d7c9e" stopOpacity="0.78" />
          ...
          {markers.length === 0 ? (
            ...
            markers.map((m, i) => (
              <motion.div
                ...
                className="flex flex-col gap-3 rounded-xl border border-[var(--border)] bg-white p-4"
              >
